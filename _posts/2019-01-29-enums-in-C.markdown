...
No need for a return, even `-Wreturn-type` (enabled in `-Wall`) will not
complain as gcc will understand that no code after the switch can be reached.

On the contrary, this version:
{% highlight C %}
bool fruit_is_valid(enum fruit fruit)
{
	switch (fruit) {
...
Would trigger the following warning:

{% highlight bash %}
warning: enumeration value ‘FRUIT_COUNT’ not handled in switch [-Wswitch]
{% endhighlight %}
